    ...
    def analysis_yaml(self, caseinfo):  # 用analysis_yml方法，来分析yml文件；传入caseinfo
        ...
        if 'name' in caseinfo_keys and 'base_url' in caseinfo_keys and 'request' in caseinfo_keys and 'validate' \
            ...
            if 'method' in request_keys and 'url' in request_keys:
                ...
                files = None
                if jsonpath.jsonpath(caseinfo, "$..file"):  # 通过jsonpath去取值；jsonpath.jsonpath(值,表达式)
                    files = caseinfo['request']['data']['file']
                ...
                response = self.send_request(method=method, url=url, headers=headers, files=files, **data)
                print(response.text)
